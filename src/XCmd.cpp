...
//	GetDirName (m_XCMDFolderSpec.vRefNum, m_XCMDFolderSpec.parID, dirname);
	...
	for	(index = 1, err = 0; err == 0; index++)
	{
		...
		if	((myCPB.hFileInfo.ioFlAttrib & ioDirMask) || (myCPB.hFileInfo.ioFlFndrInfo.fdFlags & fInvisible))
			continue;	// Verzeichnisse und unsichtbare Dateien �berspringen
		...
		{
			P2C(fName);
			DebugWarning("CXCmd::Preload() -- Fehler beim �ffnen von %s", fName+1);
...
void CXCmd::InitXCMD(uint64_t connectionId)
{
#if 0
	// TODO: implement
	OSErr err;
...
/**********************************************************************
*
* XCmd �ber GetSharedLibrary laden
*
**********************************************************************/
	...
	if	((factories != NULL) && (CFArrayGetCount(factories) > 0))
	{
		...
		if	(iunknown)
		{
			...
			if	(interfaceTable)
			{
				...
				*ppInterface = interfaceTable[0];		// wir nehmen das erste Interface, das pa�t
...
/**********************************************************************
*
* Erzeuge neuen Glue-Code f�r ein Symbol
*
**********************************************************************/
...
)
{
	...
	{
		// neuen Eintrag f�r verkettete Liste erzeugen
...
)
{
	...
	{
		//
		// Wird der Pfad angegeben, unterst�tzen wir nur CFM/PEF
		//
...
)
{
	...
	if	(XCmdDescriptor > MAX_PLUGINS)
	{
		DebugError("CXCmd::OnCommandFindSymbol()-- ung�ltiger Deskriptor", SymIndex);
		return(fnfErr);
	}
	type = s_Plugins[XCmdDescriptor].RunTimeFormat;
	if	(type == ePEF)
...
INT32 CXCmd::Command(UINT32 params, uint8_t *addrOffset68k)
{
#if 0
	// TODO: implement
