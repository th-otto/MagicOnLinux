...
//#include "Preferences.h"	// masOS GUI
...
class CMyPreferences /*: public CPreferences*/
{
	...
	*/
	const char *m_drvPath[NDRIVES];

	const char *getDrvPath(unsigned drv)
	{
		return (drv < NDRIVES) ? m_drvPath[drv] : nullptr;
	}

	void setDrvPath(unsigned drv, const char *path)
	{
		if (drv < NDRIVES)
		{
			if (m_drvPath[drv] != nullptr)
			{
				free((void *) m_drvPath[drv]);
				m_drvPath[drv] = nullptr;
			}
			if (path != nullptr)
			{
