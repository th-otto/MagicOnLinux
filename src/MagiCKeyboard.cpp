...
#if 0
const uint32_t CMagiCKeyboard::s_modconvtab[16] =
{
	cmdKey,0x37,
	shiftKey,42,
	rightShiftKey,54,
...
unsigned char CMagiCKeyboard::GetScanCode(uint32_t message)
{
	unsigned char scan;
...
unsigned char CMagiCKeyboard::GetModifierScanCode(uint32_t modifiers, bool *bAutoBreak)
{
	uint32_t diffmod;
	unsigned char ret;
	...
	// Alle Bits auf Unterschiede pr�fen
	...
	*bAutoBreak = (ret == 58);		// Atari-CapsLock
	...
	if	(modifiers & mask)
		...
		if	(!(*bAutoBreak))	// F�r CapsLock wird der Break-Code automatisch generiert
			ret |= 0x80;
		m_modifiers &= ~mask;
		}
