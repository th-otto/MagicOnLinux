...
typedef struct
{
        ...
        int     mn_menu;                /* Parent der MenÅ-Objekte */
...
#if SPECIAL_ACCMENU
/* Baum fÅr ACC/APP- MenÅ */
extern OBJECT *gem_acaptree;
/* Objektnummer des ersten MenÅs, hier gem_acaptree einsetzen */
extern int B4650E;
#endif
...
typedef struct _popupS {
  ...
  /* 0A */  GRECT pu_rect;     /* Ausmaûe des Basisobjekts */
  /* 12 */  int pu_curr_istart;
  /* 14 */  int pu_root;       /* Objekt, das Menueeintraege enthÑlt */
...
/*****************************************************************
*
* Warte auf Loslassen der Mausknîpfe
*
*****************************************************************/
...
/**********************************************************************
*
* Wenn <tree[obj]> gÅltig und nicht <ignobj> ist, wird das Objekt
...
*         RÅckgabe -1,   wenn ungÅltiges Objekt angeklickt
*                        oder Maus in MenÅzeile bewegt
*                        oder auûerhalb des MenÅs geklickt
*
***********************************************************************/
...
DEBUGSTR("enter smn_dopopup ");
     ...
     /* beim MenÅ énderung der linken Maustaste   */
     /* sonst "linke Maustaste gedrÅckt"          */
     /* ----------------------------------------- */
...
DEBUGNUM(mobj);
          ...
          if   (!quit && (which & MU_M1))
               {
               if   (openpop)      /* kann SubmenÅ îffnen */
                    {
                    openpop = popisopen = FALSE;
                    events = MU_M1|MU_BUTTON;
                    }
     
               if   (popisopen)    /* Submenue geîffnet, aber Maus noch auûerhalb */
                    {
               ...
               if   (!popisopen)        /* kein Popup ist geîffnet */
                    {
                    if   (popchild)
                         {
          ...
          /* Subpopup betreten oder Maus bewegt, wÑhrend Subpop offen */
...
#ifdef DEBUG2
DEBUGSTR("EVC ");
/* Manchmal ist <lastmobj> um 1 zu groû */
DEBUGNUM(lastmobj);
DEBUGNUM(mobj);
#endif
...
/***************************************************************************
*
* Initialisiert die Events fÅr die Bearbeitung eines Popup.
*
* Der Mauszeiger steht Åber <obj>, wobei <obj> auch NOOBJECT sein kann.
* Ist <desel> = TRUE, wird lastobj deselektiert, falls ungleich obj
*
* Wenn <obj> gÅltig ist, wird nur auf das Verlassen des Objekts
* gewartet, (MU_M1).
* Wenn dabei das Objekt auf ein Popup verweist und noch keines geîffnet
...
*    popobj = obj
*
* Wenn <obj> ungÅltig ist, wird in MU_M1 auf das Betreten des Wurzelobjekts
* gewartet.
* Bei einem MenÅ erster Ebene zusÑtzlich auf das Betreten der MenÅzeile (MU_M2).
* Wenn das Popup einen parent hat und dieser parent nicht ein normales
* MenÅ ist, wird weiterhin auf eine minimale Mausbewegung gewartet (MU_M2).
*
***************************************************************************/
                         ...
                         long *smn_timer, int desel)
{
     *events = MU_M1|MU_BUTTON;
     ...
     if   (obj != NOOBJECT)
          ...
          /* MenÅ erster Ebene: Warte auf Betreten der MenÅzeile */
          /* --------------------------------------------------- */
...
/*******************************************************************
*
* rettet/restauriert den Bildschirmhintergrund eines Popup- MenÅs
*
* <restore> = 0: retten
* sonst          restaurieren
*
* Röckgabe FALSE, wenn Fehler
*
*******************************************************************/
...
DEBUGSTR("enter smn_savescr ");
     ...
     if   (!restore)
          ...
          rect = popup->pu_rect;                  /* Auûenabmessungen */
          rect.g_x -= 1;
          rect.g_y -= 1;
          rect.g_w += 4;
          rect.g_h += 4;                     /* Schatten ???? */
          grects_intersect( &desk_g, &rect);      /* mit Bildschirm ohne MenÅ schneiden */
...
/*******************************************************************************
*
* Liefert TRUE, wenn <itree[item]> eines der beiden MenÅ-Scrollobjekte ist,
...
/*
*
* Bearbeitet die Scrollpfeile eines scrollenden MenÅs
*
*/
...
static int smn_doupdn( popupS *popup, int obj)
{
     ...
     do   {
         ...
         if    (offs)
               ...
               source_g.g_h = ob->ob_height;           /* Hîhe einer Zeile */
...
* FÅr die MenÅs. Benutzt den festen Bildschirmpuffer
*
********************/
...
/*
*
* Berechnet Parent (G_BOX) des MenÅs mit Titelobjekt <W0C>
*
* Aufbau eines MenÅs:
*
*    Objekt 0       IBOX      umfaût Bildschirm und MenÅleiste
*     Objekt 1      BOX       weiûe Box, MenÅleiste
*     Objekt 2      IBOX      Parent fÅr alle MenÅtitel
*      Objekt 3...n TITLE     MenÅtitel
*     Objekt n+1    IBOX      Bildschirm ohne MenÅleiste, Parent fÅr MenÅs
*
*/
...
static int smn_get_menu_obj( OBJECT *titletree, int titleobj )
{
     ...
     menu_obj = titletree[ titletree[ ROOT].ob_tail].ob_head;    /* erstes MenÅ */
...
/*
*
* Hintergrund in den festen Puffer retten und MenÅ zeichnen
*
* <title> ist die Objektnummer des MenÅtitels.
* Gibt die Objektnummer des MenÅs zurÅck.
* 
*/
...
/******************************************************************
*
* Hauptroutine: Anzeigen eines MenÅs.
*
* retval->tree enthÑlt bei Aufruf den MenÅbaum
*
* RÅckgabe 0, wenn MenÅ abgebrochen, d.h. nichts angewÑhlt,
* sonst != 0
*
* Röckgabewerte:
*    retvals        int       titel
*    retvals+2      int       MenÅeintrag
*    retvals+4      OBJECT *  Objektbaum (ggf. SubmenÅ)
*    retvals+8      int       Parentobjekt des MenÅs
*
* Aufbau eines MenÅs:
*    Objekt 0       IBOX      umfaût Bildschirm und MenÅleiste
*    Objekt 1       BOX       weiûe Box, MenÅleiste
*    Objekt 2       IBOX      Parent fÅr alle MenÅtitel
*    Objekt 3...n   TITLE     MenÅtitel
*    Objekt n+1     IBOX      Bildschirm ohne MenÅleiste, Parent
*                             fÅr MenÅs
*
******************************************************************/
...
int do_menu( struct domenuret *retval, void *menu_app )
{
     ...
     int dobj;
     OBJECT *menutree;
     int popupdone;           /* TRUE, falls Popup-Eintrag ausgewÑhlt */
     int res;                 /* RÅckgabewert (1 = OK, 0 = nix gewÑhlt) */
     int quit;
     int lasttitle;
     int m2obj;               /* Menuebox */
     ...
     MENU outmenu;
     MENU inmenu;
     popupS *popup;           /* heruntergefallenes MenÅ */
...
DEBUGSTR("enter smn_mgr ");
     ...
     mode = 1;
     popup = NULL;                      /* kein heruntergefallenes MenÅ */
     title = tstate = m2obj = NOOBJECT;
     xtitle = lasttitle = NOOBJECT;
     res = quit = FALSE;
     titletree = retval->tree;          /* aktuelles MenÅ */
     menutree = titletree;
     smn_reqmstate = 0x1;
...
*/
     ...
     /* Objekt 2 ist Parent fÅr alle MenÅtitel, dessen Ausmaûe */
     ...
     if   ((title != NOOBJECT) && !(titletree[title].ob_state & DISABLED))
          ...
          /* MenÅtitel selektieren und MenÅ herunterklappen */
          /* ---------------------------------------------- */
          ...
          while(!quit)
               ...
               switch(mode)
                    ...
                    /* Mauszeiger Åber MenÅtitel            */
                    /* warte auf 1. Verlassen des Titels    */
                    /*           2. Betreten des MenÅs      */
                    /* ------------------------------------ */
               ...
               if   (which & MU_BUTTON)
                    ...
                    /* Bei der Suche nicht Gesamtobjekt und MenÅleiste */
                    /* berÅcksichtigen */
                    dobj = _objc_find( titletree, 2, 1, *((long *) &mkb.evd.x));

                    /* Klick auf kein oder auf ungÅltiges Objekt */

                    if   ((dobj == NOOBJECT) ||
                         (dobj != NOOBJECT) && (titletree[ dobj].ob_state & DISABLED))
                         {
                         /* Tastaturstatus beim Auslîsen des MenÅs merken */
/*                       *pkstate = mkb.kstate;        */
                         quit = TRUE;
                         }
                    else {
               ...
               /* M2 eingetroffen, und MenÅ war offen */
               /* ----------------------------------- */

               if   (!quit && (which & MU_M2) && (popup))
                    {

                    /* heruntergefallenes MenÅ bearbeiten */
                    /* ---------------------------------- */
          ...
          if   (title != NOOBJECT)
               ...
               if   (popupdone && (outmenu.mn_item != NOOBJECT))
                    {
                    res = TRUE;
                    retval->title = title;             /* Objektnummer des MenÅtitels */
                    retval->tree = outmenu.mn_tree;    /* Baum des MenÅs */
                    retval->pmenu = outmenu.mn_menu;   /* Wurzel des MenÅs */
                    retval->menu_obj = outmenu.mn_item;     /* ausgewÑhltes Objekt */
...
DEBUGSTR("enter smn_popup ");
     ...
     /* kein MenÅ-Popup, sondern ein User-Popup */
     /* --------------------------------------- */
     ...
     /* Bei Fehler nur Nullen zurÅckgeben */
     /* --------------------------------- */
...
/*
*
* Erzeugt ein neues popupS, hÑngt es in die verkettete
...
static void smn_trudchange( popupS *popup, int W0C)
{
     ...
     smn_trudinsert( popup);       /* Scrollpfeile wieder einfÅgen */
...
* eines ScrollmenÅs, wenn dies nîtig ist.
*
**********************************************************************/
...
/***********************************************************************
* 
* Restauriert das erste und das letzte Objekt eines ScrollmenÅs
*
***********************************************************************/
