...
#define PATTOBJLEN  7
#define NAMLEN      32
/* Soviele Extraspalten fÅr einen Dateinamen: */
...
#define MINFREEBLK  4096L          /* soviel muû noch frei sein */
...
typedef struct _fi
{
     struct _fi *next;             /* Verkettung fÅr scrollbox */
     WORD      sel;                /* selektiert */
     ...
     char      is_alias;
     char      dummy;              /* fÅr name auf Wortgrenze */
     char      name[0];
} FILEINFO;
...
typedef struct
{
     ...
     OBJECT *sort_popup;      /* Objektbaum fÅr Sortiermodus */
     int  sort_mode;
     CICONBLK *folder_icon;   /* Farbicon fÅr Ordner */
     XTED xted;               /* FÅr scrollendes Eingabefeld */
     XTED xted_ext;           /*  dito fÅr Eingabe der Extension */
     char input_ext[MAXEXTLEN+2];  /* Eingabepuffer fÅr Ext. */
     char abbr_path[MAXPATH+2];    /* abgekÅrzter Pfad */

     WORD editob;
     WORD cursorpos;          /* fÅr select_item() */

     USERBLK userblk_clip_on;
     GRECT tmpclip;           /* Altes GRECT fÅr USERDEF */
     ...
     char *patterns;          /* Mîgliche Muster */
     WORD cdecl (*filter)
               (char *path,
               char *name,
               XATTR *xa);    /* statt Muster */
     char *paths;             /* Mîgliche Pfade */
     char *pattern;           /* aktuelles Muster */

     char *memblk;            /* Block mit Verzeichnisdaten */
     FILEINFO **files;        /* Zeiger auf FILEINFOs */
     FILEINFO *next_selfile;  /* fÅr RÅckgabe mehrerer Dateien */
     FILEINFO *last_selected; /* fÅr Dialogbehandlung */
     FILEINFO *last_deselected;    /* fÅr Dialogbehandlung */
     int  nfiles;             /* Anzahl Dateien */
     int  flags;              /* versch. Flags */
     int  max_name;           /* LÑngster Dateiname: Anzahl Zeichen */
     ...
     WORD button;             /* fÅr Fenster-Dialogbehandlung */
     WORD pathlen;            /* max. PfadlÑnge inkl. EOS */
     WORD fnamelen;           /* max. DateinamenlÑnge inkl. EOS */
     char *fname;             /* fÅrs Editfeld */
     char *old_fname;         /* zum Testen auf énderung */
...
/*
     4.9.96: Feste Extensions, die demnÑchst in der INF-Datei
...
extern GRECT desk_g;     /* Bildschirm ohne MenÅleiste */
...
*
* RÅckgabe:    Anzahl der benîtigten Bytes
*              *n = Anzahl der Zeichenketten
*              *maxlen = LÑnge der lÑngsten Zeichenkette
*
*********************************************************************/
...
/*********************************************************************
*
* FÅgt eine Zeichenkette <newpattern> in eine Liste <patterns>
* ein, und zwar an der Stelle <pos>. Ist <pos> == NULL, wird das
* <newpattern> angehÑngt. Gibt einen Zeiger auf das eingefÅgte
* pattern zurÅck.
...
/*********************************************************************
*
* Wandelt eine Zeichenkette in Groûschrift um.
*
*********************************************************************/
...
* vollstÑndige regulÑre AusdrÅcke mit Rekursion.
*
*********************************************************************/
...
static int pattern_match(char *pattern, char *fname)
{
     ...
     while((*pattern) == '*')
          pattern++;
     return(!(*pattern));     /* OK, wenn nur '*'e Åbrig */
}
...
/*********************************************************************
*
* FÅhrt einen Popup-Dialog aus Zeichenketten durch.
* Das Popup erscheint Åber dem Objekt parent_tree[objnr].
* Die Zeichenketten sind durch EOS getrennt und durch EOS/EOS
* abgeschlossen.
* In <strings2> kann eine zweite Zeichenkettenliste oder NULL
* Åbergeben werden.
*
*    RÅckgabe:      NULL  nix ausgewÑhlt oder dasselbe nochmal
*                   ausgewÑhlt oder zuwenig Speicher
*
*********************************************************************/
...
/*********************************************************************
*
* FÅhrt einen Popup-Dialog mit einem Objektbaum durch.
* Das Popup erscheint Åber dem Objekt parent_tree[objnr].
*
*    RÅckgabe:      NULL  nix ausgewÑhlt oder dasselbe nochmal
*                   ausgewÑhlt oder zuwenig Speicher
...
#pragma warn .pia
     ...
     /* Verzeichnis îffnen (Automounter!) */
     /* --------------------------------- */
     ...
     /* Mono-Zeichensatz: Strings fÅr den Text: */
...
/*
     if   (was_long_names)
          {
          form_alert(1,"[1][Das Verzeichnis enthÑlt Åberlange|"
                         "Dateinamen.][  OK  ]");
          }
*/
     ...
     if   (fsd->dos_mode && (fslx_flags & SHOW8P3))
          fsd->max_name = 12;           /* 8.3 */
     else fsd->max_name = 0;            /* lÑngster Dateiname */
     if   (!finfo_big.fontmono)
          {
...
/***************************************************************
*
* Der Pfad hat sich geÑndert, und der Dialog wird
...
/***************************************************************
*
* Das Suchmuster hat sich geÑndert, und der Dialog wird
* entsprechend modifiziert.
*
****************************************************************/
...
/***************************************************************
*
* Der Dateiname hat sich geÑndert, und der Dialog wird
* entsprechend modifiziert.
* Falls sich der Cursor im Dateinamenfeld befunden hat, muû
...
/***************************************************************
*
* Der Sortiermodus hat sich geÑndert, und der Dialog wird
* entsprechend modifiziert.
*
****************************************************************/
...
* RÅckgabe -1, wenn nicht sichtbar.
*
****************************************************************/
...
/***************************************************************
*
* USERBLK-Funktionen fÅr Clipping und Textausgabe
*
****************************************************************/
...
static void korr_tree( OBJECT *tree, int is_wdialog )
{
     ...
     tree->ob_width = fslx_dlw;    /* letzte Grîûe */
          ...
          if   (fsd->rshdr)
               ...
               fsd->lbox =
                    lbox_create(
                         ...
                         set_item,
                         NULL,     /* Items */
                         NLINES,   /* Anzahl sichtbarer EintrÑge */
...
/* hslider: */
                         tree[FS_BOX0].ob_width/big_wchar,
                                   /* sichtbare Spalten fÅr HSlider, inkl. Icon */
                         0,        /* H-Scrollposition */
...
/***************************************************************
*
* Zeichnet ein geÑndertes Objekt neu.
*
****************************************************************/
...
/***************************************************************
*
* Zeichnet geÑnderte Objekte neu.
*
****************************************************************/
...
/***************************************************************
*
* Hauptroutine fÅr Tastaturbedienung.
* Wird VOR Bearbeitung der Events durchgefÅhrt
* RÅckgabe 1, wenn Taste verarbeitet.
*
****************************************************************/
     ...
     if   ((key & 0xff) == '\r')   /* Return */
          ...
          if   (fsd->editob == FS_INPUT_EXT)
               ...
               new_pattern:
                    ...
                    /* Test, ob Ñndern oder anfÅgen */
                    /* d.h. s = EinfÅgeposition */
...
/****************************************************************
*
* Grîûenanpassung fÅr Dialogfenster
*
****************************************************************/
...
static void _resize_fs_tree( OBJECT *tree, WORD offset )
{
     ...
     for  (i = 0; i < NLINES; i++)
          {
          index = objs[i];         /* Box, enthÑlt Icon+Text */
          o = tree+index;
          o->ob_width = pr2;
...
*/
     ...
     /* umzusetzende Objekte an Originalposition lîschen */
...
*/
     ...
     /* Inneres neu zeichnen, wenn Scrolloffset verÑndert */
     newwidth -= oldwidth;
...
/***************************************************************
*
* Hauptroutine fÅr Button-Aktionen
*
***************************************************************/
                    ...
                    int nclicks)
{
     ...
     if   ((line >= 0) ||
          ...
          if   ((nclicks > 1) && (fsd->last_selected))
               ...
               else return(FS_OK); /* Datei zurÅckgeben */
               }
          return(0);
          }
...
/***************************************************************
*
* Callback fÅr Autolocator
* Wird nur fÅr die Dialog-Version verwendet. Wird zweimal
                         ...
                         WORD *cursorpos, WORD *editob)
{
     ...
     /* Post-Callback: evt_data enthÑlt die Event-Daten von evnt_multi */
...
* =======================
*
* Die Eingabeargumente mÅssen, wenn sie von fsel_(ex)input()
* kommen, noch gehîrig korrigiert werden (Normalisierung des
...
*    pathlen        LÑnge des Pfadpuffers inkl. EOS
*    fname          Dateiname (Ein- und Ausgabe)
*    fnamelen       LÑnge des Dateinamenspuffers inkl. EOS
*    patterns       Durch EOS getrennte, durch EOS/EOS
*                   abgeschlossene Liste von regul. AusdrÅcken
*                   Eine "oder"-VerknÅpfung wird durch ein ','
*                   angegeben, daher kann nicht nach einem ','
*                   gesucht werden.
*                   Die Liste muû (!) ein "*" enthalten.
*    filter         Funktion, die fÅr Dateien 1 oder 0 liefert
...
* Ausgabe:
*
*    fname          ausgewÑhlter Dateiname (wenn *button = 1)
*    path           ausgewÑhlter Pfad (wenn *button = 1)
*    *button        BetÑtigter Button (0 oder 1)
*    *sort_mode     Sortiermodus
*    *pattern       (Zeiger darf NULL sein) GewÑhltes Muster
*    *nfiles        Anzahl zusÑtzlicher Dateien
*
* RÅckgabe:
*
*    Zeiger         OK
...
/***************************************************************
*
* Funktionen fÅr Dateiauswahl im Fenster
* ======================================
*
****************************************************************/
#pragma warn -par
               ...
               char **pattern )    /* ist immer != NULL */
{
     ...
     if   (events->mwhich & MU_KEYBD)
          ...
          if   (((FSEL_DIALOG *) fsd)->whdl == topw)
               ...
               /* Tasten, die ich an wdlg_evnt() vorbeimogeln muû */
               /* ----------------------------------------------- */
               ...
               /* Tasten, die mîglichweise hinterher den    */
               /* Autolocator auslîsen kînnen               */
               /* ----------------------------------------- */
...
WORD fslx_close( void *fsd )
{
     if   (((FSEL_DIALOG *)fsd)->dialog)
          fslx_dlw = (((FSEL_DIALOG *)fsd)->tree)->ob_width;     /* letzte Grîûe merken */
